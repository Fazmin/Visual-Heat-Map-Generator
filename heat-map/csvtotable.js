	...
	$.fn.CSVToTable = function(csvFile, options) {
		...
		return this.each(function() {
			...
			$.get(csvFile, function(data) {
				var tableHTML = '<table id="sortnowtable" style="border:0px;border-collapse:collapse;" class="' + options.tableClass + '">';
				...
				$.each(lines, function(lineCount, line) {
					if ((lineCount == 0) && (typeof(options.headers) != 'undefined')) {
						...
						//tableHTML += '<thead><tr class="' + options.trClass + '">';
						tableHTML += '<thead class="' + options.theadClass + '"><tr class="' + options.trClass + '">';
						$.each(headers, function(headerCount, header) {
							//tableHTML += '<th class="' + options.thClass + '"><h3>' + header + '</h3></th>';
							tableHTML += '<th><h3>' + header + '</h3></th>';
						});
					...
					if ((lineCount == options.startLine) && (typeof(options.headers) == 'undefined')) {
						...
						if (items.length > 1) {
							...
							$.each(items, function(itemCount, item) {
								tableHTML += '<td title="' + item + '">' + item + '</td>';
								//tableHTML += '<td title="' + item + '" class="' + options.tdClass + '">' + item + '</td>';
							});
							tableHTML += '</tr>';
						}
					}
